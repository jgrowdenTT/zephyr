...
static int i3c_stm32_config_get(const struct device *dev, enum i3c_config_type type, void *config)
{
	...
	if (type == I3C_CONFIG_TARGET) {
		...
		config_target->enabled = (LL_I3C_GetMode(i3c) == LL_I3C_MODE_TARGET);
		config_target->dynamic_addr =
			config_target->enabled ? LL_I3C_GetOwnDynamicAddress(i3c) : 0U;
		config_target->static_addr = 0u;
		...
		config_target->supported_hdr = 0U;
#else
		return -ENOTSUP;
#endif
	}
...
static int i3c_stm32_configure(const struct device *dev, enum i3c_config_type type, void *cfg)
{
	...
	if (type == I3C_CONFIG_TARGET) {
		...
		LL_I3C_SetMIPIInstanceID(i3c, config->mipi_instance);
		LL_I3C_SetDeviceCharacteristics(i3c, config->dcr);
		LL_I3C_SetMaxReadLength(i3c, config->mrl);
		LL_I3C_SetMaxWriteLength(i3c, config->mwl);
	}
#endif
...
static void i3c_stm32_target_init(const struct device *dev, uint8_t mipi_instance)
{
	...
	/* Configure FIFO */
	LL_I3C_SetRxFIFOThreshold(i3c, HAL_I3C_RXFIFO_THRESHOLD_1_4);
	LL_I3C_SetTxFIFOThreshold(i3c, HAL_I3C_TXFIFO_THRESHOLD_1_4);
...
static int i3c_stm32_init(const struct device *dev)
{
	...
	if (config->target_mode) {
		ret = i3c_stm32_configure(dev, I3C_CONFIG_TARGET, NULL);
		if (ret != 0) {
	...
	if (LL_I3C_GetMode(i3c) == STM32_I3C_MODE_TARGET) {
		while (LL_I3C_IsActiveFlag_TXFNF(i3c)) {
			...
			if (data->target_config && data->target_config->callbacks) {
				if (data->target_config->callbacks->read_processed_cb) {
					uint8_t byte = 0;
...
#ifdef CONFIG_I3C_TARGET
	if (LL_I3C_IsActiveFlag_RXFNE(i3c) && LL_I3C_GetMode(i3c) == STM32_I3C_MODE_TARGET) {
		if (data->target_config && data->target_config->callbacks &&
...
#define I3C_STM32_INIT(index)                                                                      \
	...
	static const struct i3c_stm32_config i3c_stm32_cfg_##index = {                             \
		...
		IF_ENABLED(CONFIG_I3C_TARGET, (                                                    \
		.target_mode = DT_INST_PROP_OR(index, target_mode, false),                         \
